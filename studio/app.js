/**
 * Mersin Manşet Dashboard v5 — app.js
...
async function _draw() {
  ...
  ctx.fillText('SON DAKİKA', bdgX + 43, bdgY + bdgH / 2);
  ...
  ctx.fillText('mersinmanset.tr', innerX, panelY + panelH - 26);
...
const API_BASE    = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1' || window.location.hostname === OCI_VM_IP 
                    ? '' 
                    : `http://${OCI_VM_IP}:3000`;
...
els.btnSaveInterval.addEventListener('click', async () => {
  ...
  try {
    const res = await fetch(`${API_BASE}/api/settings/cron`, {
      method: 'POST',
      headers: {
...
async function loadStats() {
  try {
    const res   = await fetch(`${API_BASE}/posted_news.json`);
    const data  = await res.json();
...
document.getElementById('btnSaveSources').addEventListener('click', async () => {
  ...
  try {
    const res = await fetch(`${API_BASE}/api/settings/sources`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
